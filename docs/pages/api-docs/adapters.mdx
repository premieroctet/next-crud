...
An optional array of the Prisma models you want to support.
...
An object of arrays containing all the relations and nested relations of your modal that could be queried from the `where` filter. For nested relations, split them with a `.`, e.g: `posts.author`
The keys of the object must correspond to the models names.

Example:

```typescript
new PrismaAdapter<User | Post>({
  models: ['user', 'post'],
